...
import { CANONICAL_HOURS, ANGELUS_PRAYER_FULL } from '../data/prayers';
...
export default function HoursScreen() {
  ...
  const playBell = async (times) => {
    await bellService.ring(times);
  };

  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <LinearGradient colors={['#0D0D1A', '#13131F']} style={styles.header}>
        <Text style={styles.headerTitle}>📖 Kanonlu Saatler</Text>
        <Text style={styles.headerSub}>Liturgy of the Hours — Günlük Dua Programı</Text>
      </LinearGradient>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <TouchableOpacity onPress={() => setShowAngelus(true)} activeOpacity={0.8}>
          <LinearGradient colors={colors.gradientGold} style={styles.angelusButton}
            ...
            <View style={styles.angelusText}>
              <Text style={styles.angelusTitle}>Angelus Duası</Text>
              <Text style={styles.angelusSub}>Tam metni oku • 06:00 / 12:00 / 18:00</Text>
            </View>
        ...
        {/* Hours list */}
        <Text style={styles.sectionTitle}>YEDİ KANONİK SAAT</Text>
            ...
            <LinearGradient colors={[hour.color + '18', '#1C1C2E']}
              ...
              <View style={styles.cardBody}>
                <View style={styles.cardTop}>
                  <Text style={[styles.cardName, { color: hour.color }]}>{hour.turkishName}</Text>
                  <Text style={styles.cardTime}>{hour.time}</Text>
                </View>
                <Text style={styles.cardLatin}>{hour.latinName}</Text>
                <Text style={styles.cardDesc} numberOfLines={2}>{hour.description}</Text>
              </View>
        ...
        {/* Ring manually section */}
        <Text style={styles.sectionTitle}>ÇANI ÇAL</Text>
        <View style={styles.ringRow}>
          {[1, 2, 3, 5, 7].map(n => (
            <TouchableOpacity key={n} style={styles.ringButton}
              onPress={() => playBell(n)} activeOpacity={0.7}>
        ...
        <View style={styles.overlay}>
          <View style={styles.modal}>
            <LinearGradient colors={['#1C1C2E', '#0D0D1A']} style={styles.modalInner}>
              ...
              <Text style={[styles.modalTitle, { color: selected?.color }]}>{selected?.turkishName}</Text>
              <Text style={styles.modalSub}>{selected?.latinName} • {selected?.time}</Text>
              {selected?.angelus && (
                <View style={styles.angelusTag2}>
                  <Text style={styles.angelusTag2Text}>✝ ANGELUS VAKTI</Text>
                </View>
              )}
              ...
              <View style={styles.divider} />
              <Text style={styles.modalDescLabel}>Dua</Text>
              <ScrollView style={styles.prayerScroll}>
                <Text style={styles.modalPrayer}>{selected?.prayer}</Text>
              </ScrollView>
              <View style={styles.modalButtons}>
                <TouchableOpacity style={styles.bellBtn} onPress={() => bellService.ring(3)} activeOpacity={0.8}>
                  <Text style={styles.bellBtnText}>🔔 Çanı Çal</Text>
                </TouchableOpacity>
        ...
        <View style={styles.overlay}>
          <View style={styles.modal}>
            <LinearGradient colors={['#1C1C2E', '#0D0D1A']} style={styles.modalInner}>
              ...
              <Text style={[styles.modalTitle, { color: colors.primary }]}>Angelus Duası</Text>
              <Text style={styles.modalSub}>06:00 • 12:00 • 18:00</Text>
              <View style={styles.divider} />
              <ScrollView style={styles.prayerScroll}>
                <Text style={styles.modalPrayer}>{ANGELUS_PRAYER_FULL}</Text>
              </ScrollView>
